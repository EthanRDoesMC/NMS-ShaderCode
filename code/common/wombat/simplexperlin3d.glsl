...
float SimplexPerlin3D( vec3 P )
{
    ...
    vec3 Pi = floor( P + dot( P, float2vec3( SKEWFACTOR) ) );

    //  Find the vectors to the corners of our simplex tetrahedron
    vec3 x0 = P - Pi + dot(Pi, float2vec3( UNSKEWFACTOR ) );
    ...
    vec3 Pi_inc1 = step( Pi, float2vec3( 69.0 - 1.5 ) ) * ( Pi + 1.0 );
    ...
    //	evaluate gradients
    vec4 grad_results = invsqrt( hash_0 * hash_0 + hash_1 * hash_1 + hash_2 * hash_2 ) * ( hash_0 * v1234_x + hash_1 * v1234_y + hash_2 * v1234_z );
